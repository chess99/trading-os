...
def run_canslim_screen(
    ...
    *,
    ...
    for idx, candidate in enumerate(candidates[:top_n], 1):
        candidate["rank"] = idx
    return _finish_result(hub, run, "canslim_screen", trace, candidates[:top_n], filtered)
...
def _finish_result(
    ...
    report = _canslim_report(candidates, filtered)
    hub.store.write_run_artifacts(
        run,
        manifest=manifest,
...
def _canslim_report(candidates: list[dict[str, Any]], filtered: dict[str, int]) -> str:
    lines = ["# CANSLIM Screen", "", f"Candidates: {len(candidates)}", "", "## Filtered Out"]
    ...
    for candidate in candidates:
        ...
        lines.append(f"- {rank}. {symbol} {name} score={score}")
    return "\n".join(lines) + "\n"
