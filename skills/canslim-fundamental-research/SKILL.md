...
- `elder-confirm`：技术面确认，现在能不能进场（CANSLIM 体系专用）
...
建议下一步：
→ {如果值得关注：运行 elder-confirm 确认技术面入场时机}
...
从 recipe 输出读取 `manifest.json`、`trace.md` 和 `report.md`。CANSLIM 重点是成长质量和候选队列；估值只能作为辅助约束，且必须来自 recipe 产物或明确标注数据不足。
...
| 命令 | 输出内容 |
|------|---------|
| `research company --template canslim` | 成长质量、估值口径、风险、限制、manifest |
